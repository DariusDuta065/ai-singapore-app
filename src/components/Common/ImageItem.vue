...
export default {
  ...
  methods: {
    ...
    getSrc(ext = null) {
      ...
      return require(`@img/${this.image}/${this.image}_w_${srcW}.${ext}`);
    },
    getSrcSet(ext = null) {
      const { srcSet, exts } = this.images[this.image];
  ...
  mounted: function() {
    ...
    const loadImg = () => {
      const imgEl = Array.from(this.$el.children).find(el => el.nodeName === 'IMG');
      imgEl.src = imgEl.dataset['url'];

      const sourceEl = Array.from(this.$el.children).find(el => el.nodeName === 'SOURCE');
      console.log('Found sources', sourceEl);
